...
//[ECID:000000] ZTEBSP jiaobaocun 20120820 start, for FM open rf power when fm on
#ifdef CONFIG_RADIO_EXT_LNA
...
extern char *get_modelno(void);
static int iris_fops_open(struct file *file)
{
	int rc = 0;
	char *modelon;
	modelon = get_modelno();
	if(strcmp(modelon,"wfgA")==0) 
	{
	/* Voting for 2.8V Regulator to enable fm rf power*/
	fm_regulator = regulator_get(NULL , "8921_l15");
	...
	/* Set the voltage level to 2.8V */
	rc = regulator_set_voltage(fm_regulator, 2800000, 2800000);
	if (rc < 0) {
		pr_err("%s: could not set voltage: %d\n", __func__, rc);
		goto reg_free;
	}

	/* Enabling the 2.8V regulator */
	...
	if (rc) {
		...
		{
		pr_debug("%s: fm antenna power switch has enabled!\n",__func__);
	}
reg_free:
...
static int iris_fops_release(struct file *file)
{
	...
	if (!IS_ERR_OR_NULL(fm_regulator)) {
		...
		if (rc)
			...
			pr_debug("%s:fm antenna power switch has disabled!\n",__func__);
		regulator_put(fm_regulator);
		fm_regulator = NULL;
	}
	}
...
#endif
//[ECID:000000] ZTEBSP jiaobaocun 20120820 end, for FM open rf power when fm on
