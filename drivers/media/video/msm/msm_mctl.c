			...
			unsigned int cmd, unsigned long arg)
{
    ...
    // zte-modify, 20130121 fuyipeng modify for module ID +++
    case MSM_CAM_IOCTL_GET_SENSOR_MODULE_ID:
    {
        uint16_t ModuleId = 0; 
        struct msm_sensor_ctrl_t *s_ctrl = get_sctrl(p_mctl->sensor_sdev);
        pr_err("%s: Get ModuleId \n", __func__);

        if (s_ctrl->func_tbl->sensor_get_module_id != NULL)
        {
            ModuleId = s_ctrl->func_tbl->sensor_get_module_id(s_ctrl);
        }

        *(uint16_t *)arg = ModuleId;
        break;
    }
    // zte-modify, 20130121 fuyipeng modify for module ID ---
