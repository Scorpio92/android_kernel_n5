...
pm8xxx_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	//printk("PM_DEBUG_MXP: Enter pm8xxx_rtc_set_time.\n");
	//printk("PM_DEBUG_MXP: Here before rtc_tm_to_time: tm->tm_year=%d, tm->tm_mon=%d, tm->tm_mday=%d\n",tm->tm_year,tm->tm_mon,tm->tm_mday);
	//printk("PM_DEBUG_MXP: Here before rtc_tm_to_time: tm->tm_wday=%d, tm->tm_yday=%d.\n",tm->tm_wday,tm->tm_yday);
	//printk("PM_DEBUG_MXP: Here before rtc_tm_to_time: tm->tm_hour=%d, tm->tm_min=%d, tm->tm_sec=%d\n",tm->tm_hour,tm->tm_min,tm->tm_sec);
	rtc_tm_to_time(tm, &secs);
	//printk("PM_DEBUG_MXP: Here after rtc_tm_to_time: secs=%ld.\n",secs);
	value[0] = secs & 0xFF;
	value[1] = (secs >> 8) & 0xFF;
	...
	//printk("PM_DEBUG_MXP:Seconds value to be written to RTC = %lu.\n",secs);
	
	spin_lock_irqsave(&rtc_dd->ctrl_reg_lock, irq_flags);
	ctrl_reg = rtc_dd->ctrl_reg;
	//printk("PM_DEBUG_MXP: ctrl_reg=%d.\n",ctrl_reg);
	if (ctrl_reg & PM8xxx_RTC_ALARM_ENABLE) 
	{
		//printk("PM_DEBUG_MXP: Can go ctrl_reg & PM8xxx_RTC_ALARM_ENABLE.\n");
		alarm_enabled = 1;
...
rtc_rw_fail:
	if (alarm_enabled)
		spin_unlock_irqrestore(&rtc_dd->ctrl_reg_lock, irq_flags);
	//printk("PM_DEBUG_MXP: Exit pm8xxx_rtc_set_time.\n");
	return rc;
}
...
pm8xxx_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	//printk("PM_DEBUG_MXP: Enter pm8xxx_rtc_set_alarm.\n");
	rtc_tm_to_time(&alarm->time, &secs);
	//printk("PM_DEBUG_MXP: alarm secs = %ld.\n",secs);
	/*
	...
	if (rc < 0) 
	{
		printk("PM_DEBUG_MXP:Unable to read RTC time.\n");
		return -EINVAL;
	}
	//printk("PM_DEBUG_MXP: pm8xxx_rtc_read_time: rtc_tm.tm_year=%d, rtc_tm.tm_mon=%d, rtc_tm.tm_mday=%d\n",rtc_tm.tm_year,rtc_tm.tm_mon,rtc_tm.tm_mday);
	//printk("PM_DEBUG_MXP: pm8xxx_rtc_read_time: rtc_tm.tm_wday=%d, rtc_tm.tm_yday=%d.\n",rtc_tm.tm_wday,rtc_tm.tm_yday);
         //printk("PM_DEBUG_MXP: pm8xxx_rtc_read_time: rtc_tm.tm_hour=%d, rtc_tm.tm_min=%d, rtc_tm.tm_sec=%d\n",rtc_tm.tm_hour,rtc_tm.tm_min,rtc_tm.tm_sec);
	rtc_tm_to_time(&rtc_tm, &secs_rtc);
	//printk("PM_DEBUG_MXP: rtc secs = %ld.\n",secs_rtc);
	if (secs < secs_rtc) 
	{
		printk("PM_DEBUG_MXP:Trying to set alarm in the past,not allowed.\n");
		return -EINVAL;
	}
	...
	if (rc < 0) 
	{
		printk("PM_DEBUG_MXP:Write to RTC ALARM registers failed.\n");
		goto rtc_rw_fail;
	}
	...
	if (rc < 0) 
	{
		printk("PM_DEBUG_MXP:PM8xxx write rtc alarm failed.\n");
		goto rtc_rw_fail;
	}
...
static int
pm8xxx_virtual_alarm_set(struct device *dev, struct rtc_wkalrm *a)
{
	//struct pm8xxx_rtc *rtc_pdata = dev_get_drvdata(dev);
	unsigned long now = get_seconds();
	unsigned long secs;
	int diff;
	
	//printk("PM_DEBUG_MXP: Enter pm8xxx_virtual_alarm_set.\n");
	//printk("PM_DEBUG_MXP: now_seconds is:now_time %ld\n",now);
	if (!a->enabled) 
	{
		secs = 0;
		printk("PM_DEBUG_MXP:a->enabled=0.\n");
		return 0;
	} else
		rtc_tm_to_time(&a->time, &secs);
	
	//printk("PM_DEBUG_MXP: alarm secs = %ld.\n",secs);
         diff = secs-now;
	  
	if (now > secs) {
		printk("PM_DEBUG_MXP: Attempt to set alarm in the past.\n");
		secs = 0;
		return -EINVAL;
	}
	else
	{
	printk("PM_DEBUG_MXP:set max sleep time diff=%d.\n",diff);
	msm_pm_set_max_sleep_time((int64_t)
			((int64_t) diff * NSEC_PER_SEC));
	//msmrtc_suspend (dev,PMSG_SUSPEND);
	}
	//printk("PM_DEBUG_MXP: Exit pm8xxx_virtual_alarm_set.\n");
	return 0;
}
...
static struct rtc_class_ops pm8xxx_rtc_ops = {
	...
	.set_alarm	= pm8xxx_virtual_alarm_set,
...
static int __devinit pm8xxx_rtc_probe(struct platform_device *pdev)
{
	...
	printk("PM_DEBUG_MXP: Enter pm8xxx_rtc_probe.\n");
	printk("PM_DEBUG_MXP: rtc_write_enable = %d.\n",rtc_write_enable);
	...
	printk("PM_DEBUG_MXP: the return of pm8xxx_read_wrapper:rc = %d.\n",rc);
	...
	rc = pm8xxx_write_wrapper(rtc_dd, &ctrl_reg, rtc_dd->rtc_base, 1);
	printk("PM_DEBUG_MXP: the return of pm8xxx_write_wrapper:rc = %d.\n",rc);
	if (rc < 0) {
		dev_err(&pdev->dev, "PM8xxx write failed!\n");
		goto fail_rtc_enable;
	}
	...
	printk("PM_DEBUG_MXP: Exit pm8xxx_rtc_probe.\n");
	return 0;
...
static void pm8xxx_rtc_shutdown(struct platform_device *pdev)
{
	...
	//printk("PM_DEBUG_MXP: Enter pm8xxx_rtc_shutdown.\n");
	if (pdata != NULL)
...
fail_alarm_disable:
	...
	//printk("PM_DEBUG_MXP: Exit pm8xxx_rtc_shutdown.\n");
}
