...
/*[ECID:000000] ZTEBSP zhangbo save soc before lowpower shutdown, start*/
void zte_backup_soc( int soc)
{
	backup_soc_and_iavg(the_chip, 200, soc);
}
/*[ECID:000000] ZTEBSP zhangbo save soc before lowpower shutdown, end*/
...
/*[ECID:000000] ZTEBSP zhangbo, fix index jumping, start*/
/*
 * Avoid battery level jump
 */
static int zte_state_of_charge_filter(int last_soc, int soc, int delta_time_us)
{
	int ret = last_soc;
	static int delta_time_unchanged = 0;
	struct timespec ts = {0,0};
	static struct timespec pre_change_ts= {0,0};
	int allow_jump_len=1;

	if(soc < 10 ||last_soc == -EINVAL)
		return soc;

	delta_time_unchanged += delta_time_us;

	pr_debug("delta_time_unchanged=%d\n",delta_time_unchanged);

	if(last_soc != soc) {
		get_monotonic_boottime(&ts);
		pr_debug("(ts.ts_sec - pre_change_ts.ts_sec)=%ld\n",(ts.tv_sec - pre_change_ts.tv_sec));

		if(delta_time_unchanged > 20*USEC_PER_SEC || (ts.tv_sec - pre_change_ts.tv_sec) > 600) {
			
			//allow change 1 level every 10 minutes if suspend
			allow_jump_len = (ts.tv_sec - pre_change_ts.tv_sec -delta_time_unchanged/USEC_PER_SEC)/600;
			...
			if(soc > last_soc)
				ret = (last_soc + allow_jump_len) <soc ? (last_soc + allow_jump_len) : soc;
			else
				ret = (last_soc - allow_jump_len) > soc ? (last_soc - allow_jump_len) : soc;

			delta_time_unchanged = 0;
			pre_change_ts.tv_sec = ts.tv_sec;
			}
	}
	return ret;
}
/*[ECID:000000] ZTEBSP zhangbo, fix index jumping, end*/
...
static int report_state_of_charge(struct pm8921_bms_chip *chip)
{
	...
	/*[ECID:000000] ZTEBSP zhangbo, fix index jumping, start*/
	//last_soc = soc;
	last_soc = zte_state_of_charge_filter(last_soc, soc, delta_time_us);
	/*[ECID:000000] ZTEBSP zhangbo, fix index jumping, end*/
...
desay:
		...
		return 0; //ZTEBSP zhangbo
/*[ECID:000000] ZTBSP zhangbo change for charging, start*/
zte:
		chip->fcc = zte_battery_data.fcc;
		chip->fcc_temp_lut = zte_battery_data.fcc_temp_lut;
		chip->pc_temp_ocv_lut = zte_battery_data.pc_temp_ocv_lut;
		chip->pc_sf_lut = zte_battery_data.pc_sf_lut;
		chip->rbatt_sf_lut = zte_battery_data.rbatt_sf_lut;
		chip->default_rbatt_mohm = zte_battery_data.default_rbatt_mohm;
		chip->delta_rbatt_mohm = zte_battery_data.delta_rbatt_mohm;
		return 0;
