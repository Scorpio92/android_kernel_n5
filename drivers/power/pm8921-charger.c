...
/*[ECID:000000] ZTEBSP zhangbo add chg feature, start*/
#define ZTE_CHG_WAKE_LOCK
#define ZTE_CONFIG_PM8921_LOW_BATTERY_ALARM
/*[ECID:000000] ZTEBSP zhangbo add chg feature, end*/

/*ZTEBSP zhangbo change for low voltage battery alarm, start*/
#ifdef ZTE_CONFIG_PM8921_LOW_BATTERY_ALARM
#include <linux/mfd/pm8xxx/batt-alarm.h>
extern void kernel_power_off(void);
#endif
/*ZTEBSP zhangbo change for low voltage battery alarm, end*/

#ifdef ZTE_CONFIG_PM8921_LOW_BATTERY_ALARM
	#define ZTE_BATTERY_ALARM_CHECK_HYSTERSIS_MS  2000
	#define ZTE_BATTERY_POWEROFF_CHECK_TIMES 5
#endif

//#define ZTE_CHG_USBIN_VOLTAGE_DETETCT

void zte_power_supply_changed(struct power_supply *psy);
...
//			bool				keep_btm_on_suspend; //[ECID:000000] ZTEBSP zhangbo add qualcomm pathc, replace btm with adc algorithm 
	bool				ext_charging;
	...
	struct work_struct batt_alarm_work;
	struct delayed_work batt_alarm_hystersis_work;
	#endif
	/*ZTEBSP zhangbo change for low voltage battery alarm, end*/
	/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, start*/
	#ifdef ZTE_CHG_WAKE_LOCK
	struct wake_lock chg_lock;
	struct work_struct chg_lock_work;
	#endif
	/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, end*/
	#ifdef ZTE_CHG_USBIN_VOLTAGE_DETETCT
	struct delayed_work usbin_detect_work;
	#endif
};
...
/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, start*/
 ...
 */
 struct zte_chg_capacity_filter_data_type {
 	struct timespec chg_charging_start;
	struct timespec pre_change_ts;
	struct timespec low_batt_volt_start_ts; 
	long charging_time;
	int last_soc;
	bool chg_full_pending;
	int batt_voltage;
	spinlock_t lock;
 };
 ...
 //extern void zte_backup_soc( int soc);
/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, end*/
//static struct pm8xxx_adc_arb_btm_param btm_config; //[ECID:000000] ZTEBSP zhangbo add qualcomm patch, replace btm with adc algorithm

static int get_prop_batt_status(struct pm8921_chg_chip *chip);//ZTEBSP zhangbo add declaration
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		/* USB charging */
		/*[ECID:000000] ZTEBSP zhangbo, dc charge greater than USB_WALL_THRESHOLD_MA, so add equals here, start*/
		if (usb_target_ma <= USB_WALL_THRESHOLD_MA)
			val->intval = is_usb_chg_plugged_in(the_chip);
		else
		    return 0;
		break;
		/*[ECID:000000] ZTEBSP zhangbo, dc charge greater than USB_WALL_THRESHOLD_MA, so add equals here, end*/
 ...
 * Report 0% if the battery voltage is less than 3V.
 */
static int zte_chg_capacity_filter(int percent_soc)
{
	int battery_status;
	int soc;
	...
	if(the_chip == NULL) {
		filterP->last_soc = soc;
		return soc;
	}
	
	battery_voltage_mv = get_prop_battery_uvolts(the_chip)/1000;
	...
	//correct soc while charging full.
	battery_status = get_prop_batt_status(the_chip);
	...
	if(filterP->last_soc != soc) {
		...
		if((ts.tv_sec - filterP->pre_change_ts.tv_sec) > (soc>60?60:(soc<20?20:soc))) {
			...
			//allow change 1 level every 5mins
			allow_jump_len = (ts.tv_sec - filterP->pre_change_ts.tv_sec)/300;
			
			if(allow_jump_len <= 0)
				allow_jump_len = 1;
				
				
			if((soc > filterP->last_soc) &&(is_usb_chg_plugged_in(the_chip) || is_usb_chg_plugged_in(the_chip))) {
				filterP->last_soc = (filterP->last_soc + allow_jump_len) <soc ? (filterP->last_soc + allow_jump_len) : soc;
				filterP->pre_change_ts.tv_sec = ts.tv_sec;
			}
			else if(soc < filterP->last_soc) {
				filterP->last_soc = (filterP->last_soc - allow_jump_len) > soc ? (filterP->last_soc - allow_jump_len) : soc;
				filterP->pre_change_ts.tv_sec = ts.tv_sec;
			}
		}
	}
	return filterP->last_soc;
...
static int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
{
	...
	/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, start*/
		percent_soc = zte_chg_capacity_filter(percent_soc);
	/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, end*/
...
/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, start*/
static int zte_get_prop_batt_status(struct pm8921_chg_chip *chip)
{
	int batt_state = POWER_SUPPLY_STATUS_DISCHARGING;
	batt_state = get_prop_batt_status(chip);
	return batt_state;	
}
/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, end*/
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, start*/
		val->intval = zte_get_prop_batt_status(chip);
		/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, end*/
		break;
...
static void __pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	/*ZTEBSP zhangbo change for usb charger current, start*/
	/*[ECID:000000] ZTEBSP zhangbo change for invalid charger or power line, start*/
	if(the_chip->usb_psy.type == POWER_SUPPLY_TYPE_UNKNOWN) {
		if(is_usb_chg_plugged_in(the_chip) || is_dc_chg_plugged_in(the_chip)) {
			pr_debug("POWER_SUPPLY_TYPE_UNKNOWN\n");
			mA =500;
		}
	}
	/*[ECID:000000] ZTEBSP zhangbo change for invalid charger or power line, end*/
	 if((the_chip->usb_psy.type == POWER_SUPPLY_TYPE_UNKNOWN ||the_chip->usb_psy.type == POWER_SUPPLY_TYPE_USB ) && (mA > 500))
		mA = 500;
	 pr_debug("charger type: %d mA=%d\n\n",the_chip->usb_psy.type, mA);
	/*ZTEBSP zhangbo change for usb charger current, end*/
...
void pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	if (the_chip) {
		//ZTEBSP zhangbo change current
		if ((mA > USB_WALL_THRESHOLD_MA) &&(the_chip->usb_psy.type == POWER_SUPPLY_TYPE_USB))
...
int pm8921_set_usb_power_supply_type(enum power_supply_type type)
{
	...
	zte_power_supply_changed(&the_chip->usb_psy);
	zte_power_supply_changed(&the_chip->dc_psy);
	return 0;
}
...
static void handle_usb_insertion_removal(struct pm8921_chg_chip *chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		zte_power_supply_changed(&chip->usb_psy);
		zte_power_supply_changed(&chip->batt_psy);
		pm8921_bms_calibrate_hkadc();
	}
	/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, start*/
 	zte_chg_capacity_filter_data.chg_full_pending = false;
	/*[ECID:000000] ZTEBSP correct capacity according battery voltage and charging status, end*/
	if (usb_present) {
		schedule_delayed_work(&chip->unplug_check_work,
			round_jiffies_relative(msecs_to_jiffies
				(UNPLUG_CHECK_WAIT_PERIOD_MS)));
		pm_chg_auto_enable(chip, true); /*[ECID:000000] ZTEBSP zhangbo change for invalid charger or power line*/
		pm8921_chg_enable_irq(chip, CHG_GONE_IRQ);
	} else {
		/* USB unplugged reset target current */
		usb_target_ma = 0;
		pm8921_chg_disable_irq(chip, CHG_GONE_IRQ);
		/*[ECID:000000] ZTEBSP zhangbo change for usb charger, start*/
		the_chip->usb_psy.type = POWER_SUPPLY_TYPE_UNKNOWN;
		/*[ECID:000000] ZTEBSP zhangbo change for usb charger, end*/
	}
...
static void handle_stop_ext_chg(struct pm8921_chg_chip *chip)
{
	...
	zte_power_supply_changed(&chip->dc_psy);
...
static irqreturn_t usbin_valid_irq_handler(int irq, void *data)
{
	...
	pr_info("usbin_valid_irq_handler entered.");
	/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, start*/
	#ifdef ZTE_CHG_WAKE_LOCK
	schedule_work(&the_chip->chg_lock_work);
	#endif
	/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, end*/
...
{
/*   2012-10-18   Modify begin for low input charger(100-500mA)  charge normal*/
...
static void unplug_check_worker(struct work_struct *work)
{
	...
	if (the_chip->usb_psy.type == POWER_SUPPLY_TYPE_USB)
	{
		...
		usb_target_ma=1500;
	else if(the_chip->usb_psy.type == POWER_SUPPLY_TYPE_UNKNOWN) {
		if(is_usb_chg_plugged_in(the_chip) || is_dc_chg_plugged_in(the_chip)) {
			usb_target_ma =0;
			__pm8921_charger_vbus_draw(500);
		}
	...
	/*[ECID:000000] ZTEBSP zhangbo change for invalid charger or power line, start*/
	/**
	  * With power line, only is ground and power line connected, the PBL_ACCESS1 register returns 0x84.
	  * With usb or wall charger, the PBL_ACCESS1 register returns 0xa4.
	  */
	//active_chg_plugged_in = is_active_chg_plugged_in(chip, active_path);
	active_chg_plugged_in = is_active_chg_plugged_in(chip, active_path|USB_ACTIVE_BIT);
	/*[ECID:000000] ZTEBSP zhangbo change for invalid charger or power line, end*/
	...
	if (active_path & USB_ACTIVE_BIT) {
		...
		/*[ECID:000000] ZTEBSP zhangbo Qualcomm patch, new procedure for reverse boost fix, start*/
...
/*[ECID:000000] ZTEBSP zhangbo add qualcomm patch, replace btm with adc algorithm, start*/
...
/*[ECID:000000] ZTEBSP zhangbo add qualcomm patch, replace btm with adc algorithm, end*/
enum {
	CHG_IN_PROGRESS,
	CHG_NOT_IN_PROGRESS,
	CHG_FINISHED,
...
/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, start*/
#ifdef ZTE_CHG_WAKE_LOCK
static void zte_chg_lock_work(struct work_struct *work)
{
	struct pm8921_chg_chip *chip=	container_of(work, struct pm8921_chg_chip, chg_lock_work);
	if(is_usb_chg_plugged_in(chip) || is_dc_chg_plugged_in(chip))
		wake_lock(&chip->chg_lock);
	else
		wake_lock_timeout(&chip->chg_lock,5*HZ);
}
#endif
/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, end*/

#ifdef ZTE_CHG_USBIN_VOLTAGE_DETETCT
static void zte_usbin_detect_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct pm8921_chg_chip *chip=	container_of(dwork, struct pm8921_chg_chip, usbin_detect_work);
	int rc;
	struct pm8xxx_adc_chan_result result;
	...
	if(result.physical == 0) {
		...
		if((result.physical == 0) && (!is_usb_chg_plugged_in(chip)) && (!is_dc_chg_plugged_in(chip)))
		{
			pm8921_turn_on_19p2mhz_clk(chip, false);
		}
	}
adc_error:
	schedule_delayed_work(&chip->usbin_detect_work,
			      round_jiffies_relative(msecs_to_jiffies
						     (1000)));
}
#endif
/*[ECID:000000] ZTEBSP don't update power supply change many times in a short time, start*/
struct zte_power_supply_report_data_type {
	struct delayed_work		update_work;
	struct power_supply *psy[3];
	spinlock_t lock;
	int start;
};
struct zte_power_supply_report_data_type zte_power_supply_report_data;
static void zte_power_supply_schedule_worker(struct work_struct *work)
{
	struct power_supply *psy;
	int index;
	struct delayed_work *dwork = to_delayed_work(work);
	struct zte_power_supply_report_data_type *pdata = container_of(dwork,
				struct zte_power_supply_report_data_type, update_work);
	spin_lock(&pdata->lock);
	for(index=0;index<3;index++) {
		if(pdata->psy[(pdata->start+index)%3]) {
			psy = pdata->psy[(pdata->start+index)%3];
			pdata->psy[(pdata->start+index)%3] = NULL;
			pdata->start = (pdata->start+index+1)%3;
			spin_unlock(&pdata->lock);
			power_supply_changed(psy);
			schedule_delayed_work(&pdata->update_work,HZ/2);
			return;
		}
	}
	spin_unlock(&pdata->lock);
...
void zte_power_supply_changed(struct power_supply *psy)
{
	static unsigned long pre_time = 0; //report change last time

	spin_lock(&(zte_power_supply_report_data.lock));
	if(jiffies -  pre_time > HZ/2) {
		pre_time = jiffies;
		spin_unlock(&(zte_power_supply_report_data.lock));
		power_supply_changed(psy);
	}
	else
	{
		if(psy == &the_chip->batt_psy)
			zte_power_supply_report_data.psy[0] = psy;
		else if(psy == &the_chip->usb_psy)
			zte_power_supply_report_data.psy[1] = psy;
		else if(psy == &the_chip->dc_psy)
			zte_power_supply_report_data.psy[2] = psy;
		spin_unlock(&(zte_power_supply_report_data.lock));
		schedule_delayed_work(&(zte_power_supply_report_data.update_work), HZ/2);
	}
}
/*[ECID:000000] ZTEBSP don't update power supply change many times in a short time, end*/

/*ZTEBSP zhangbo change for low voltage battery alarm, start*/
#ifdef ZTE_CONFIG_PM8921_LOW_BATTERY_ALARM
static int pm8921_low_battery_alarm_notify(struct notifier_block *nb,
					  unsigned long status, void *unused);

static struct notifier_block low_battery_alarm_notifier = {
	.notifier_call = pm8921_low_battery_alarm_notify,
};
//extern void zte_pwrkey_input(void);
static void zte_batt_alarm_work(struct work_struct *work)
{
       static int low_times = 0;
	struct timespec ts;
	static struct timespec pre_ts={0,0};
	int current_voltage_mv;
       struct pm8921_chg_chip *chip=	container_of(work, struct pm8921_chg_chip, batt_alarm_work);
	...
	if((pm_chg_get_rt_status(chip, USBIN_VALID_IRQ) == false) && (pm_chg_get_rt_status(chip, DCIN_VALID_IRQ) == false)) {
		current_voltage_mv = get_prop_battery_uvolts(chip)/1000;
		if(current_voltage_mv < 3000) {
			ktime_get_ts(&ts);
			if((ts.tv_sec- pre_ts.tv_sec) > (ZTE_BATTERY_ALARM_CHECK_HYSTERSIS_MS/1000+2))
				low_times=0;
			else
				low_times++;
			if(low_times >= ZTE_BATTERY_POWEROFF_CHECK_TIMES) {
				//zte_backup_soc(0);
				kernel_power_off();
			}
			pre_ts.tv_sec=ts.tv_sec;
		} else
			low_times=0;
	} else
		low_times=0;
		
       schedule_delayed_work(&chip->batt_alarm_hystersis_work,msecs_to_jiffies(ZTE_BATTERY_ALARM_CHECK_HYSTERSIS_MS));

}

static void batt_alarm_hystersis_worker(struct work_struct *work)
{
	int rc;
	pr_info("%s entered\n",__func__);
       rc = pm8xxx_batt_alarm_enable(
			PM8XXX_BATT_ALARM_LOWER_COMPARATOR);//only enable low battery alarm 

       if (rc) {
		printk("%s: unable to set alarm state\n", __func__);
      	}
		...
		unsigned long status, void *unused)
{
	...
	wake_lock_timeout(&chip->batt_alarm_wake_lock, 5 * HZ);
	schedule_work(&chip->batt_alarm_work);
	
		break;
	case 2:
		printk("%s: trip of high threshold\n", __func__);
		break;
	default:
		printk("%s: error received\n", __func__);
	};
...
{
	//int rc; /*[ECID:000000] ZTEBSP zhangbo add qualcomm patch, replace btm with adc algorithm */
	struct pm8921_chg_chip *chip = dev_get_drvdata(dev);
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	//chip->keep_btm_on_suspend = pdata->keep_btm_on_suspend; //[ECID:000000] ZTEBSP zhangbo add qualcomm patch, replace btm with adc algorithm
	...
	INIT_DELAYED_WORK(&(zte_power_supply_report_data.update_work), zte_power_supply_schedule_worker);
	...
	/*ZTEBSP zhangbo change for low voltage battery alarm, start*/
	#ifdef ZTE_CONFIG_PM8921_LOW_BATTERY_ALARM
	wake_lock_init(&chip->batt_alarm_wake_lock, WAKE_LOCK_SUSPEND,"zte_batt_alarm");

	INIT_WORK(&chip->batt_alarm_work, zte_batt_alarm_work);
	INIT_DELAYED_WORK(&chip->batt_alarm_hystersis_work,
						batt_alarm_hystersis_worker);
	...
	if (!rc)
		rc = pm8xxx_batt_alarm_disable(
			PM8XXX_BATT_ALARM_LOWER_COMPARATOR);
	if (rc) {
		printk("%s: unable to set batt alarm state\n", __func__);
		goto free_irq;
	}
	...
	rc = pm8xxx_batt_alarm_threshold_set(
			PM8XXX_BATT_ALARM_LOWER_COMPARATOR, 3300);
	if (!rc)
		rc = pm8xxx_batt_alarm_threshold_set(
			PM8XXX_BATT_ALARM_UPPER_COMPARATOR, 4450);//compatible to high voltage battery
	if (rc) {
		printk("%s: unable to set batt alarm threshold\n", __func__);
		goto free_irq;
	}
	...
	if (rc) {
		printk("%s: unable to set batt alarm hold time\n", __func__);
		goto free_irq;
	}
	...
	if (rc) {
		printk("%s: unable to set batt alarm pwm rate\n", __func__);
		goto free_irq;
	}

	rc = pm8xxx_batt_alarm_register_notifier(&low_battery_alarm_notifier);
	if (rc) {
		printk("%s: unable to register alarm notifier\n", __func__);
		goto free_irq;
	}
	 ...
	 */
	/*[ECID:000000] ZTEBSP zhangbo add qualcomm, replace btm with adc algorithm, start*/ 
	//if (!(chip->cool_temp_dc == INT_MIN && chip->warm_temp_dc == INT_MIN)) {
	...
	//	}
	//}
	/*[ECID:000000] ZTEBSP zhangbo add qualcomm, replace btm with adc algorithm, end*/ 
	/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, start*/
	#ifdef ZTE_CHG_WAKE_LOCK
	wake_lock_init(&chip->chg_lock, WAKE_LOCK_SUSPEND,
		       "zte_chg");
	INIT_WORK(&chip->chg_lock_work, zte_chg_lock_work);
	#endif
	/*[ECID:000000] ZTEBSP zhangbo add chg wakelock, end*/
	
	#ifdef ZTE_CHG_USBIN_VOLTAGE_DETETCT
	INIT_DELAYED_WORK(&chip->usbin_detect_work,
						zte_usbin_detect_work);
	schedule_delayed_work(&chip->usbin_detect_work,
			      round_jiffies_relative(msecs_to_jiffies
						     (10000)));
	#endif
...
static int __devexit pm8921_charger_remove(struct platform_device *pdev)
{
	...
	if (!rc)
		rc = pm8xxx_batt_alarm_disable(
			PM8XXX_BATT_ALARM_LOWER_COMPARATOR);
	if (rc)
		printk("%s: unable to set low batt alarm state\n", __func__);

	rc |= pm8xxx_batt_alarm_unregister_notifier(&low_battery_alarm_notifier);
	if (rc)
		printk("%s: unable to register low batt alarm notifier\n", __func__);
	#endif
	/*ZTEBSP zhangbo change for low voltage battery alarm, end*/
