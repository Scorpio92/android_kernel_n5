...
static int sys_mode = 1;

int ztemt_get_sys_mode(int arg) {
	return sys_mode;
}

static ssize_t RF_MODE_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
...
static ssize_t RF_MODE_store(struct device *pdev, struct device_attribute *attr,
			    const char *buff, size_t size)
{
	sscanf(buff, "%d", &sys_mode);
	return size;
}
#endif
...
#ifdef CONFIG_ZTEMT_MODEM_SUPPORT
static DEVICE_ATTR(RF_MODE, S_IRUGO | S_IWUSR, RF_MODE_show, RF_MODE_store);
#endif
