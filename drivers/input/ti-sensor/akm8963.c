...
static irqreturn_t akm8963_irq(int irq, void *handle)
{
	...
	if ((buffer[0] & 0x01) != 0x01) {
       ...
       //printk("fanjiankang: goes to akm8963_irq\n");
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (s_akm->irq == 0) {
		...
		printk("fanjiankang: probe goes to akm no irq\n");
	} else {
		...
		printk("fanjiankang: probe goes to akm  irq\n");
	}
...
static void create_akm_info_proc_file(void)
{
...
static void remove_akm_info_proc_file(void)
{
	//printk("goes to remove_acc_info_proc_file\n");
	if(akm_info_proc_file){
		remove_proc_entry("driver/compass", NULL);
		akm_info_proc_file = NULL;
	}
}
...
static int __init akm8963_init(void)
{
	...
	err = i2c_add_driver(&akm8963_driver);
	if(0 == err){
		create_akm_info_proc_file();	
	}	
	return err;
}
