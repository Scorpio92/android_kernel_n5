...
//[ECID:0000]ZTE_BSP wangminrong 20120730 modify lcd off for poweroff charger start 
//extern int set_lcd_backlight(int bl);
//extern int bootup_mode;
//[ECID:0000]ZTE_BSP wangminrong 20120730 modify lcd off for poweroff charger end
...
static int taos_get_data(void)//iVIZM
{
    ...
    if ((ret = (i2c_smbus_write_byte(taos_datap->client, (TAOS_TRITON_CMD_REG | 0x00)))) < 0) {
        printk(KERN_ERR "TAOS: i2c_smbus_write_byte(1) failed in taos_get_data()\n");
        return (ret);
    }
...
#endif
static void taos_input_poll_func(struct input_polled_dev *dev)
{
    wake_lock(&taos_datap->taos_wake_lock);
    taos_get_data();
    wake_unlock(&taos_datap->taos_wake_lock);
...
static int taos_als_get_data(void)//iVIZM
{
	...
	input_report_abs(taos_datap->input_poll_dev->input,ABS_MISC,lux_val);
	input_sync(taos_datap->input_poll_dev->input);//[ECID:000000] ZTEBSP wanghaifei 20111019, input SYNC
    return ret;
}
...
static int taos_prox_threshold_set(void)//iVIZM
{
	...
	if (prox_on || proxdata < taos_cfgp->prox_threshold_lo ) {
		...
		data = 0;
		input_report_abs(taos_datap->input_poll_dev->input,ABS_DISTANCE,data);
...
//[ECID:000000] ZTEBSP wanghaifei start 20120416, add sys attr for product test
...
#define LSM330D_ACC_I2C_SLAVE_ADDR          0x19
#define LSM330D_ACC_WHO_AM_I                    0x0F
#define LSM330D_ACC_ID                                  0x33

#define LSM330D_GYRO_I2C_SLAVE_ADDR       0x6a
#define LSM330D_GYRO_WHO_AM_I                 0x0F
#define LSM330D_GYRO_ID                               0xd4

#define AK8963_I2C_SLAVE_ADDR                   0x0c
...
static ssize_t taos_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	 ...
	 union i2c_smbus_data data;
        int result = -1;
#if 0	
#if defined ( CONFIG_PROJECT_P825A10 ) || defined ( CONFIG_PROJECT_P825F01 ) || defined ( CONFIG_PROJECT_P865F01 )
        struct i2c_adapter *mpu_adapter = i2c_get_adapter(1);
	result = sensor_i2c_read(mpu_adapter, L3G4200D_GYR_I2C_SLAVE_ADDR, L3G4200D_GYR_WHO_AM_I, 1, &mpu_id);
#else
        struct i2c_adapter *mpu_adapter = i2c_get_adapter(0);
	result = sensor_i2c_read(mpu_adapter, MPU_I2C_SLAVE_ADDR, MPUREG_WHO_AM_I, 1, &mpu_id);
#endif
#endif
       struct i2c_adapter *taos_adapter = i2c_get_adapter(5);
...
#endif
#endif
       struct i2c_adapter *gyro_adapter = i2c_get_adapter(2);
	result = sensor_i2c_read(gyro_adapter, LSM330D_GYRO_I2C_SLAVE_ADDR, LSM330D_GYRO_WHO_AM_I, 1, &gyro_id);

	i2c_put_adapter(gyro_adapter);
...
#if 0
#if defined ( CONFIG_PROJECT_P825A10 ) || defined ( CONFIG_PROJECT_P825F01 ) || defined ( CONFIG_PROJECT_P865F01 )
        struct i2c_adapter *accel_adapter = i2c_get_adapter(1);
	result = sensor_i2c_read(accel_adapter, LIS3DH_I2C_SLAVE_ADDR, LIS3DH_WHO_AM_I, 1, &accel_id);
#else
        struct i2c_adapter *accel_adapter = i2c_get_adapter(0);
...
#endif
#endif
       struct i2c_adapter *accel_adapter = i2c_get_adapter(2);
	result = sensor_i2c_read(accel_adapter, LSM330D_ACC_I2C_SLAVE_ADDR, LSM330D_ACC_WHO_AM_I, 1, &accel_id);
...
#if 0		
#if defined ( CONFIG_PROJECT_P825A10 ) || defined ( CONFIG_PROJECT_P825F01 ) || defined ( CONFIG_PROJECT_P865F01 )
        struct i2c_adapter *compass_adapter = i2c_get_adapter(1);
	result = sensor_i2c_read(compass_adapter, AK8962_I2C_SLAVE_ADDR_A10, AK8962_WHO_AM_I, 1, &compass_id);
#else
        struct i2c_adapter *compass_adapter = i2c_get_adapter(0);
	result = sensor_i2c_read(compass_adapter, AK8962_I2C_SLAVE_ADDR, AK8962_WHO_AM_I, 1, &compass_id);
#endif
#endif
	 struct i2c_adapter *compass_adapter = i2c_get_adapter(2);
	 result = sensor_i2c_read(compass_adapter, AK8963_I2C_SLAVE_ADDR, AK8963_WHO_AM_I, 1, &compass_id);
...
//[ECID:000000] ZTEBSP wanghaifei end 20120416, add sys attr for product test
...
/*[ECID:000000] ZTEBSP wanghaifei start 20111018, move disable_irq to taos_open ,change exit sequence */
    ...
    kfree(taos_cfgp);
    input_unregister_polled_device(taos_datap->input_poll_dev);
...
static int taos_probe(struct i2c_client *clientp, const struct i2c_device_id *idp) {
	...
	//[ECID:0000]ZTE_BSP wangminrong 20120730 modify lcd off for poweroff charger start 	
 //   if(bootup_mode == 1 || bootup_mode == 2)   /* [ECID:0000]ZTE_BSP yuanjinxing, close backlight after the second LOGO, 2012.08.01 */
	//{
...
//[ECID:000000] ZTEBSP wanghaifei start 20120416, add sys file for sensor detect
...
//[ECID:000000] ZTEBSP wanghaifei end 20120416, add sys file for sensor detect
...
/*[ECID:000000] ZTEBSP wanghaifei end 20111021*/
 ...
 //   init_MUTEX(&taos_datap->update_lock); //[ECID:000000] ZTEBSP wanghaifei  20111021, useless
   ...
   taos_datap->input_poll_dev = input_allocate_polled_device();//iVIZM
   if (taos_datap->input_poll_dev == NULL) {
/*[ECID:000000] ZTEBSP wanghaifei start 20111021, should release some memory before return */
	ret = -EBUSY;
	goto malloc2_fail;
       //return -ENOMEM;
/*[ECID:000000] ZTEBSP wanghaifei end 20111021*/
   ...
   taos_datap->input_poll_dev->private = taos_datap;
   taos_datap->input_poll_dev->poll = taos_input_poll_func;
   taos_datap->input_poll_dev->poll_interval = 200;
   taos_datap->input_poll_dev->poll_interval_max = 200;

   taos_datap->input_poll_dev->input->id.bustype = BUS_I2C;
   taos_datap->input_poll_dev->input->name = TAOS_INPUT_NAME;
   taos_datap->input_poll_dev->input->dev.parent = &taos_datap->client->dev;
   input_set_drvdata(taos_datap->input_poll_dev->input, taos_datap);
/*[ECID:000000] ZTEBSP wanghaifei start 20111020, should use input_set_abs_params if suppoet EV_ABS, or NULL pointer will be reported*/
   set_bit(EV_ABS,taos_datap->input_poll_dev->input->evbit);
/*
...
*/
    input_set_abs_params(taos_datap->input_poll_dev->input, ABS_DISTANCE, 0, 100, 0, 0);
    input_set_abs_params(taos_datap->input_poll_dev->input, ABS_MISC, 0, 20000, 0, 0);
    if ((ret = input_register_polled_device(taos_datap->input_poll_dev))) {
...
malloc3_fail:
   ...
   goto malloc2_fail;
input_register_fail:
   input_free_polled_device(taos_datap->input_poll_dev);
malloc2_fail:
i2c_func_fail:
   device_destroy(taos_class, MKDEV(MAJOR(taos_dev_number), 0));
...
static int taos_sensors_als_on(void) {
    ...
    //int lux_val = 0, ret = 0, i = 0, tmp = 0;
            ...
            #if 0 // changed by yangze for input poll for taos 20121019 
	     ...
	     // changed by yangze for input poll for taos 20121019 start 	
	     ...
	     // changed by yangze for input poll for taos 20121019 end 
