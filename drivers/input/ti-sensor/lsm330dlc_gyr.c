...
#define DEBUG 1           //added by fanjiankang
...
static int lsm330dlc_hw_init(struct lsm330dlc_data *gyro)
{
	...
	pr_info("%s hw init\n", LSM330DLC_GYR_DEV_NAME);
	// added by yangze for gyro product test 20121122
	buf[0] = WHO_AM_I;
	err = lsm330dlc_i2c_read(gyro, buf, 1);
	if (err < 0) {
		printk(KERN_ERR "Error reading lsm330_gyro WHO_AM_I\n");
		return err;
	} 
	
	if (buf[0] != WHOAMI_LSM330DLC) {
		printk(KERN_ERR "Error reading lsm330_gyro WHO_AM_I,the id is 0x%x\n",buf[0]);
...
static void create_gyro_info_proc_file(void)
{
...
#endif
	err = i2c_add_driver(&lsm330dlc_driver);
	if(0 == err){
		create_gyro_info_proc_file();	
	}
	return err;
}
