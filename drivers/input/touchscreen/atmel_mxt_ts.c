...
#define CONFIG_HAS_EARLYSUSPEND_ATMEL_MXT
#define GPIO_TOUCH_RST_OUT 33               //haoweiwei add
#ifdef CONFIG_HAS_EARLYSUSPEND_ATMEL_MXT
...
/* Define for T8 Flag*/
#define MXT_PALM_RECOVERY   1
#define MXT_PALM_NORMAL_USE 0
...
#define MXT_STATUS_CALIBRATE  (1 << 4)
...
static int mxt_T8_set_palm(struct mxt_data *data, u8 flag);
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	int touchid;
	u8 reportid;
    u8 id;              //haoweiwei add to declear id
	do {
		if (mxt_read_message(data, &message)) {
			dev_err(dev, "Failed to read message\n");
			ts_hardware_reset();                        //haoweiwei add
			goto end;
		}
			...
			&& reportid <= object->max_reportid) 
		{
			...
			{
				// Count add to 1000, the palm should be normal. set T8 to Normal USE. 
				//mxt_T8_set_palm (MXT_PALM_NORMAL_USE);
				mxt_T8_set_palm (data,MXT_PALM_NORMAL_USE);        //haoweiwei add
			}							
		...
		else if (reportid == 12)  //Key Array Process 
		{
			g_count_T9_message++;
			if (g_count_T9_message > RECOVERY_MESSAGE_COUNT)
			{
        ...
        else if (reportid == 1)   //COMMAND Process
		{
			...
			if ((message.message[0] & MXT_STATUS_CALIBRATE) || (message.message[0] & MXT_STATUS_RESET))
			{
				dev_dbg(dev, "mXT status reset: %d calibrate: %d\n", 
					         (message.message[0] & MXT_STATUS_CALIBRATE), (message.message[0] & MXT_STATUS_RESET));    	
...
static int mxt_T8_set_palm(struct mxt_data *data, u8 flag)
{
//	struct device *dev = &data->client->dev;          //because of unused haoweiwei delcet it
	u8 error = 0;
	
	if (flag)
	{		
		//Set to 0, 0, 1, 0 for Palm recovery
		error = mxt_write_object(data, MXT_GEN_ACQUIRE_T8, MXT_ACQUIRE_ATCHCALST, 0);
		if (error)
		return 1;
		
		error = mxt_write_object(data, MXT_GEN_ACQUIRE_T8, MXT_ACQUIRE_ATCHCALSTHR, 0);
		if (error)
		return 1;
		
		error = mxt_write_object(data, MXT_GEN_ACQUIRE_T8, MXT_ACQUIRE_ATCHFRCCALTHR, 1);
		if (error)
		return 1;
		
		error = mxt_write_object(data, MXT_GEN_ACQUIRE_T8, MXT_ACQUIRE_ATCHFRCCALRATIO, 0);
		if (error)
		return 1;
	}
	else
	{
		...
		error = mxt_write_object(data, MXT_GEN_ACQUIRE_T8, MXT_ACQUIRE_ATCHCALST, 255);
		if (error)
		return 1;
		
		error = mxt_write_object(data, MXT_GEN_ACQUIRE_T8, MXT_ACQUIRE_ATCHCALSTHR, 1);
		if (error)
		return 1;
		
		error = mxt_write_object(data, MXT_GEN_ACQUIRE_T8, MXT_ACQUIRE_ATCHFRCCALTHR, 0);
		if (error)
		return 1;
...
{
	struct device *dev = &data->client->dev;
//	int timeout_counter = 0;          //because of unused
	int ret;
//	u8 command_register;          //because of unused
	...
	g_count_T9_message = 0;
    ret = mxt_T8_set_palm(data, MXT_PALM_RECOVERY);
	if (ret)
	{
		dev_err(dev, "Set T8 Error!\n");
		return ret;
	}
...
/*************************************virtual keys**********************************************/
#define virtualkeys virtualkeys.Atmel maXTouch Touchscreen

static const char ts_keys_size[] = "0x01:158:197:2040:200:140:0x01:172:541:2040:200:140:0x01:139:888:2040:200:140";
...
static int ts_key_report_atmel_mxt_init(void)
{
	int ret;
		...
		const struct i2c_device_id *id)
{
	...
	data->irq = client->irq;
	/************haoweiwei add********************/
	data->init_hw = pdata->init_platform_hw;
	if(NULL != data->init_hw)
		error = data->init_hw(data->client);
/************haoweiwei add********************/
...
/*haoweiwei add for virtual keys start*/
printk("max_x = %d max_y = %d\n",data->max_x,data->max_y);
data->max_y = data->max_y - 101;
/*haoweiwei add for virtual keys end*/
    ...
    ts_key_report_atmel_mxt_init();
	ts_hardware_reset();    //haoweiwei add for reset test
	return 0;
...
err_free_mem:
	...
	kfree(data);
	ts_hardware_reset();    //haoweiwei add for reset
	return error;
}
...
#endif
  ts_key_report_deinit();    //haoweiwei add
	kfree(data->object_table);
	kfree(data);
...
static int mxt_resume(struct device *dev)
{
	...
	struct mxt_data *data = i2c_get_clientdata(client);
	struct input_dev *input_dev = data->input_dev;
	struct mxt_finger *finger = data->finger;         //清除手指数据
	int ret = 0;
    u8 id;              //haoweiwei add to clear id
	for(id=0; id < MXT_MAX_FINGER; id++)
	{
		finger[id].status = 0;                        //清除手指数据
		input_report_key(data->input_dev,ABS_MT_TRACKING_ID,id);
		input_report_abs(data->input_dev,ABS_MT_TOUCH_MAJOR, 0);
		input_report_abs(data->input_dev, ABS_MT_POSITION_X, 0);
		input_report_abs(data->input_dev, ABS_MT_POSITION_Y, 0);
