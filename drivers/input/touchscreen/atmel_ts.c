...
----------     -----------            -----------     --------------------------                
2011/12/15     wangweiping0001      wangweiping      add touchscreen driver for msm8960
...
#include <linux/proc_fs.h>/**get touchscreen information  */
...
#define ANDROID_VITUAL_KEYS     //shihuiqin add

#define GET_TOUCHSCREEN_INFORMATION/** ZTE_MODIFY  added for get touchscreen infomation */

static int debug = DEBUG_TRACE;
static int comms = 0; 

/** ZTE_MODIFY  added for atmel 224 touchscreen calibration */
...
#ifdef ANDROID_VITUAL_KEYS
/*virtual key support begin*/
static ssize_t atmel_vkeys_show(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{

	/*key_type : key_value : center_x : cener_y : area_width : area_height*/
	return snprintf(buf,200,
	__stringify(EV_KEY) ":" __stringify(KEY_MENU) ":70:840:60:40"
	":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE) ":181:840:60:40"
	":" __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":292:840:60:40"
	":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":413:840:60:40"
	"\n");
}
static struct kobj_attribute atmel_vkeys_attr = {
	.attr = {
		.mode = S_IRUGO,
	},
	.show = &atmel_vkeys_show,
};

static struct attribute *atmel_properties_attrs[] = {
	&atmel_vkeys_attr.attr,
	NULL
};
...
struct kobject *kobj_atmel;
static void ts_key_report_atmel_init(void)
{
	int rc = -EINVAL;

	/* virtual keys */
	atmel_vkeys_attr.attr.name = "virtualkeys.Atmel Touchscreen";
	kobj_atmel = kobject_create_and_add("board_properties",
				NULL);
	if (kobj_atmel)
		rc = sysfs_create_group(kobj_atmel,
...
static const u8	*object_type_name[] = {
        ...
        [44] = "SPT_MESSAGECOUNT_T44",
         [46] = "SPT_CTE_CONFIGURATION_T46",
         [47] = "PROCI_STYLUS_T47",
         [48] = "PROCI_NOISESUPPRESSION_T48",       
         [52] = "TOUCH_PROXIMITY_KEY_T52",        
         [53] = "GEN_DATA_SOURCE_T53",
};
...
/** ZTE_MODIFY yuehongliang added check calibration for 10" touchscreen, yuehongliang0005 */
...
/** ZTE_MODIFY end yuehongliang0005 */
...
/** ZTE_MODIFY yuehongliang added check calibration for 10" touchscreen, yuehongliang0005 */
        ...
        {
            if(debug >= DEBUG_TRACE)
                printk(KERN_INFO "Atmel mxt768e check chip calibration.\n");
            error = read_atch_tch_num(mxt, 32, 24, &tch_ch, &atch_ch);
            if(error < 0)
            {
                printk(KERN_ERR "Atmel mxt768e read tch and atch error.\n");
                return;
            }
        }
        else
...
#ifndef ANDROID_VITUAL_KEYS
//shihuiqintest
       u8 finger_status;
       int x_pos;
     if(mxt->finger_data[0].y > 810)
     {
              if(!mxt->finger_count)
              {
                      finger_status=0;
		...
		else
              {
	             ...
	             x_pos = mxt->finger_data[0].x;
			if((x_pos > 30)&&(x_pos < 80))
			{
				printk("press menu, finger = %d\n",finger_status);
				input_report_key(mxt->input, KEY_MENU, finger_status);
			}	
			else if ((x_pos > 150)&&(x_pos < 200))
			{
				printk("press home, finger = %d\n",finger_status);
				input_report_key(mxt->input, KEY_HOMEPAGE, finger_status);
			}
			else if ((x_pos > 270)&&(x_pos < 320))
			{
				printk("press back, finger = %d\n",finger_status);
				input_report_key(mxt->input, KEY_BACK, finger_status);
			}
			else if ((x_pos > 380)&&(x_pos < 430))
			{
				printk("press search, finger = %d\n",finger_status);
				input_report_key(mxt->input, KEY_SEARCH, finger_status);
			}
			else
			{
                           printk("press no key, finger = %d\n",finger_status);
			}
			//input_sync(mxt->input);
}
else
...
#endif
{
        ...
        {
                input_report_abs(mxt->input, ABS_MT_TOUCH_MAJOR, 0);
                input_report_key(mxt->input, BTN_TOUCH, 0);//zhangqi add for mutl
                input_mt_sync(mxt->input); 
		//  input_sync(mxt->input);	   
        }
        else
        {
                for (loop_i = 0; loop_i < mxt->numtouch; loop_i++) 
                {
                        if (mxt->finger_pressed & BIT(loop_i)) 
                        {
                        	    input_report_key(mxt->input, BTN_TOUCH, 1);//zhangqi add for mutl
                                input_report_abs(mxt->input, ABS_MT_TRACKING_ID, loop_i+1); 
                                input_report_abs(mxt->input, ABS_MT_TOUCH_MAJOR, mxt->finger_data[loop_i].z);//
                                input_report_abs(mxt->input, ABS_MT_WIDTH_MAJOR, mxt->finger_data[loop_i].w);//ABS_MT_WIDTH_MAJOR
...
//				    input_report_key(mxt->input, BTN_TOUCH, 0);//zhangqi add for mutl
                                ...
                                input_mt_sync(mxt->input); 
				    printk("x=%d y=%d \n", mxt->finger_data[loop_i].x, mxt->finger_data[loop_i].y);
				    printk("z=%d w=%d \n", mxt->finger_data[loop_i].z,  mxt->finger_data[loop_i].w);				
                        }
                } 
        ...
        if(mxt->finger_count)
        {
		  ...
		  printk("x=%d y=%d \n",mxt->finger_data[0].x,mxt->finger_data[0].y);
        }
        else
        {
                input_report_key(mxt->input, BTN_TOUCH, 0);
...
static void msg_process_finger_data(struct atmel_finger_data *fdata, u8  *data)
{
	 ...
	 fdata->x >>= 2;//zhangqi add 
        fdata->y >>= 2;
        fdata->w = data[MXT_MSG_T9_TCHAREA];
        fdata->z = data[MXT_MSG_T9_TCHAMPLITUDE];
}

/** ZTE_MODIFY , modified for the way of report touch, 2011-03-17 */
static void process_T9_message(u8 *message, struct mxt_data *mxt)
{
        ...
        if((touch_number >= 0) && (touch_number < mxt->numtouch))
        {
                ...
                else if (status & (MXT_MSGB_T9_DETECT|MXT_MSGB_T9_PRESS))  
                {
                        ...
                        /** ZTE_MODIFY  added for atmel 224 and 1386 touchscreen calibration */
                        if(mxt->pre_data[0] < 2)
                        {
				  ...
				  u8 *id_block_data)
{
	 ...
	 printk("mxt variant_id=%x  x_size=%d y_siza=%d \n", mxt->device_info.variant_id, mxt->device_info.x_size,  mxt->device_info.y_size);
                                                                ...
                                                                struct mxt_data *mxt)
{
        ...
        struct mxt_platform_data_zte *pdata;

	printk("mxt_check_config_data \n");
        ...
        for (i = 0; i < mxt->device_info.num_objs; i++) 
        {
                ...
                switch(object_type){
                        ...
                        case MXT_PROCI_TOUCHSUPPRESSION_T42:
                                calculate_config_data_crc(&my_config_crc, pdata->config_T42, object_size);
                                break;
                        ...
                        case MXT_SPT_CTE_CONFIGURATION_T46:
                                calculate_config_data_crc(&my_config_crc, pdata->config_T46, object_size);
                                break;
                                
                        case MXT_PROCI_NOISESUPPRESSION_T48:
                                calculate_config_data_crc(&my_config_crc, pdata->config_T48, object_size);
                                break;
                        default:
                ...
                {
                        printk( "the config is not matched, object_type:%d, chip crc:%d, crc:%d\n", 
                                                                ...
                                                                struct mxt_data *mxt)
{
	...
	printk("mxt_reload_config_data \n");
        ...
        for (i = 0; i < mxt->device_info.num_objs; i++) 
        {
                object_type = mxt->object_table[i].type;
                object_address = mxt->object_table[i].chip_addr;
		  printk("type =%d address=%x \n",object_type,object_address);
                if(!IS_CONFIG_OBJECT(object_type))
                {        
                        continue;
                }
                switch(object_type){
                        ...
                        case MXT_PROCI_TOUCHSUPPRESSION_T42:
                                error = mxt_write_block(mxt->client, 
                                                        get_object_address(mxt, MXT_PROCI_TOUCHSUPPRESSION_T42), 
                                                        get_object_size(mxt, MXT_PROCI_TOUCHSUPPRESSION_T42), 
                                                        pdata->config_T42);
                                break;
                        case MXT_SPT_DIGITIZER_T43:
                                error = mxt_write_block(mxt->client, 
                        ...
                        case MXT_SPT_CTE_CONFIGURATION_T46:
                                error = mxt_write_block(mxt->client, 
                                                        get_object_address(mxt, MXT_SPT_CTE_CONFIGURATION_T46), 
                                                        get_object_size(mxt, MXT_SPT_CTE_CONFIGURATION_T46), 
                                                        pdata->config_T46);
                                break;
                        case MXT_PROCI_NOISESUPPRESSION_T48:
                                error = mxt_write_block(mxt->client, 
                                                        get_object_address(mxt, MXT_PROCI_NOISESUPPRESSION_T48), 
                                                        get_object_size(mxt, MXT_PROCI_NOISESUPPRESSION_T48), 
                                                        pdata->config_T48);
                                break;
                        }
                        if (error < 0) 
...
/** ZTE_MODIFY yuehongliang added for get touchscreen infomation, yuehongliang0006 */
...
#endif
/** ZTE_MODIFY end yuehongliang0006 */
			       ...
			       const struct i2c_device_id *id)
{
	...
	if (debug >= DEBUG_TRACE)
        ...
        mxt->pre_data[0] = 0;  /** ZTE_MODIFY  added for calibration */
...
//wangbing, temp for toushscreen debug
//	if (mxt->power_on != NULL)
//		mxt->power_on(1);
//        msleep(50);
        /*maXTouch driver identifying chip*/
	...
	input_set_capability(input, EV_KEY, KEY_MENU);
        ...
        input_set_abs_params(input, ABS_MT_TOUCH_MAJOR , 0, MXT_MAX_TOUCH_SIZE,//ABS_MT_TOUCH_MAJOR
        		     0, 0);
        input_set_abs_params(input, ABS_MT_WIDTH_MAJOR , 0, MXT_MAX_WIDTH_SIZE,//ABS_MT_WIDTH_MAJOR
        		     0, 0);
...
printk( "mxt check config data CRC error =%x\n",error);
        if(error < 0)
        {
                printk( "Error check config data CRC\n");
        }
        
        if(!mxt->configuration_crc)
        {
                printk( "Touchscreen CRC Check Fail!, reload the Config data to chip\n");
        ...
        if (mxt->irq) 
        {
			...
			IRQF_TRIGGER_LOW | IRQF_ONESHOT, client->dev.driver->name, mxt);
...
static int __devexit mxt_remove(struct i2c_client *client)
{
        ...
        destroy_workqueue(mxt->atmel_wq);     //added???
...
/** ZTE_MODIFY liyuan FTM modem does not load atmel ts , 2011-08-29*/
